...
def test_verify_transactin(setup_transaction):
    transaction:Transaction = pd.read_pickle(PKL_FILE_PATH)
    from_pub_key = VerifyingKey.from_string(binascii.unhexlify(transaction.sender), curve=SECP256k1)
    signature = binascii.unhexlify(transaction.signature)
    unsigned = transaction.to_unsigned().to_dict()
